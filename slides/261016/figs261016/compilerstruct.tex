...
\begin{tikzpicture}
  ...
  \path[<->,color=red] (2.5,0) edge [above,sloped] node {\color{red}{\bf フロントエンド}} (2.5,-4);
  \path[<->,color=red] (2.5,-4) edge [above,sloped] node {\color{red}{\bf バックエンド}} (2.5,-9);
  \draw[color=red] (SRC) -- (2.75,0) (TREE) -- (2.75,-4) (OBJCODE) -- (2.75,-9);

  \syntacticObject{記号表}{symtab}{(-3.5,-4)};
  \path[->,out=180,in=60,thick] (PAR) edge (symtab);
  \path[->,out=300,in=180,thick] (symtab) edge (ILGEN);
\end{tikzpicture}
\end{document}
